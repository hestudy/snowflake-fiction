...
| **humanize-text** | AI文本人语化处理（24种检测模式+灵魂注入）⭐优化 | 人语化、去AI味、润色 |
...
| **character-check** | 角色质量检查 | 角色检查、人设检查、人物检查 |
...
# 构思期（步骤1-2）⭐新增
/outline-concept                               # 引导式构思
/outline-concept 都市重生，主角是中年失业者       # 直接输入创意
/outline-concept 玄幻 废材少年                   # 指定题材
/outline-concept --step1-only 都市职场故事       # 仅生成一句话概括

# 角色设计（步骤3+5+7）⭐新增
/character-design 主角 林小满                   # 完整深化（卡片→背景→宝典）
/character-design 快速 反派 冷酷商人             # 仅生成人物卡片
/character-design 新增配角 神秘商人              # 新增配角
/character-design 更新 林小满 --深化背景         # 更新已有角色
/character-design --from 03-完整大纲.md         # 基于大纲设计角色

# 场景规划（步骤8+9）⭐新增
/scene-plan                                    # 全量规划（基于完整大纲）
...
```bash
# 内容质量评估
/quality-check [文本]                           # 内容质量评估（冲突/情绪/期待感/节奏/钩子）
...
```bash
# 触发 outline-concept（⭐新增）
"构思故事"
"验证创意"
"故事概念"
"帮我想一个故事"

# 触发 character-design（⭐新增）
"角色设计"
"人物设计"
"新增角色"
"深化角色"
...
### 二、人语化处理（humanize-text）⭐优化

**为什么需要**：AI文本有明显的"机器味"，容易被平台检测

**v2.0 重大更新**：
- 整合 Wikipedia "Signs of AI writing" 24种检测模式
- 新增"注入灵魂"理念：不只是去除AI味，更要注入人味
- 5大模式分类：内容模式、语言模式、风格模式、交流模式、填充模糊
...
Claude：正在分析AI痕迹（24种模式）...

        发现问题：
        【内容模式】
        - #1 意义膨胀："标志着...里程碑"出现2次
        - #3 浅层-ing分析："体现了"、"反映了"出现3次
        - #5 模糊归因："专家认为"无具体来源

        【语言模式】
        - #7 AI高频词汇：深入探讨、至关重要、综上所述
        - #10 三规则滥用：出现了"创新性、实用性、美观性"组合
        - #11 同义词轮替：主角/主人公/英雄反复替换
        ...
        | 原文 | 修改后 | 模式# |
        |------|--------|-------|
        | 这一政策的出台，标志着行业发展进入了新阶段 | 这个政策2024年3月出台 | #1 |
        | 专家认为，这一技术将发挥重要作用 | 清华大学2024年报告显示... | #5 |
        | 他深入探讨了这个问题 | 他把这事儿琢磨透了 | #7 |
        | 综上所述 | 所以说 | #7 |
        | 这不仅仅是一次更新，而是... | 这次更新变化很大 | #9 |
```
...
Claude：【AI程度检测报告】
        ...
        ## 最像AI的3个片段：
        1. "在当今社会，随着科技的快速发展..."（#1意义膨胀 + #7 AI词汇）
        2. "综上所述，我们应该..."（#24通用结尾）
        3. "这一点至关重要"（#7高频AI词）
...
**内容质量评估**：从冲突强度、情绪密度、期待感、节奏控制、钩子设计五个维度打分
...
Claude：【内容质量评估报告】
        ...
        | 维度 | 得分 | 满分 | 评级 | 说明 |
        |------|------|------|------|------|
        | 冲突强度 | 14/25 | 25 | 及格 | 冲突存在但强度不足 |
        | 情绪密度 | 16/25 | 25 | 良好 | 情绪有表达，共情偏弱 |
        | 期待感 | 12/20 | 20 | 及格 | 结尾缺少悬念 |
        | 节奏控制 | 10/15 | 15 | 良好 | 节奏基本合理 |
        | 钩子设计 | 10/15 | 15 | 及格 | 开头抓力不足 |
...
1. 初始化
   ...
   或分步：/outline-concept → /character-design → 生成大纲 → /scene-plan

2. 规划阶段（一次性）
   ├─ /outline-concept [创意]          ← 步骤1-2：构思期
   ├─ /character-design 主角/反派       ← 步骤3+5+7：人物深化
   ├─ 生成大纲（outline-builder agent） ← 步骤4+6：大纲构建
   ├─ /scene-plan                      ← 步骤8+9：场景规划
   └─ 黄金三章/付费卡点/升级体系设计
...
6. 周期回顾（每周）
   ...
   └─ 调整后续大纲（outline-builder agent 重建）
```
...
| 场景 | 推荐命令 | 说明 |
|------|----------|------|
| 快速验证创意 | `/outline-concept` | 步骤1-2，生成一句话概括+五句式大纲 ⭐新增 |
...
| 角色塑造检查 | `/character-check` | 人设扁平化、工具人 |
| 选题创意检查 | `/concept-check` | 辨识度、题材匹配 |
| 人语化处理 | `/humanize-text` | 去AI味+注灵魂（24模式检测）|
