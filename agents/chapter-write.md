...
2. Read 读取写作风格配置：[小说目录]/00-写作风格.md（获取笔风参数，如文件不存在则跳过）
3. Read 读取场景规划：[小说目录]/08-场景规划/[对应场景文件]
4. Read 读取人物宝典：[小说目录]/06-人物宝典/（本章相关角色）
5. Read 读取前一章正文：[小说目录]/正文/第[N-1]章.md（保持连贯，第1章跳过此步）
6. 按场景规划生成本章正文（2000-4000字），严格遵守写作风格配置中的所有参数
7. 执行流水账自检（冲突/情绪/期待感/节奏 4维度）
8. 如果自检不通过，修正后再保存
9. 用 Write 保存到 [小说目录]/正文/第[N]章.md
10. 返回章节摘要（标题、字数、核心事件、结尾钩子类型）

小说目录：[路径]
章节编号：第[N]章
场景规划文件：[文件路径]
写作风格文件：[小说目录]/00-写作风格.md
```
