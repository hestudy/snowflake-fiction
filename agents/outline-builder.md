...
- 如果 `00-一句话概括.md` 中记录了时间线类型（非顺叙），按对应结构生成大纲：
  - **圆环**：章节规划中标注每章所属时间线（现在/过去/未来），三线交织推进
  - **读档制**：章节规划中标注循环轮次，每轮循环有新信息增量
  - **纯循环**：章节规划中标注循环编号和本轮的微小变化
  - **多线并行**：章节规划中标注每章所属线，各线独立推进并在关键章节交汇
- 生成伏笔清单（埋设章节 → 收束章节）
...
5. **时间线一致性**：如果使用非顺叙时间线，检查时间线切换是否有锚点、各线占比是否合理、信息揭露顺序是否符合该时间线类型的规则
