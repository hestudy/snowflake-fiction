...
**参数说明**：
- `--batch`：启用批量转换模式（默认行为）
- `--concurrency N`：并发数量限制，默认为 **3**（避免 API 限流）

**处理流程**：
1. 读取目录下所有 `.md` 文件
2. 按文件名排序（第1章.md, 第2章.md...）
3. **分批处理**：按并发数分批，每批处理完成后等待 1 秒再处理下一批
4. **按章节导出**：每章独立保存为 `.txt` 文件到 `./导出/[平台名]/` 目录
...
```bash
# 使用默认并发数（3）
/novel-export 番茄 ./正文/ --batch

# 自定义并发数（5个同时处理）
/novel-export 起点 ./正文/ --batch --concurrency 5

# 保守模式（单线程，最稳定）
/novel-export 晋江 ./正文/ --batch --concurrency 1
```

### 禁止合并文件

**重要**：批量导出时，**必须**按章节分别导出为独立文件。不支持 `--merge` 参数。

如果用户确实需要合并，请在所有章节导出完成后，手动使用系统命令：
```bash
cat 导出/番茄小说/*.txt > 我的小说_完整版.txt
```
...
6. **文件夹命名**：必须使用固定的平台映射名称（见上方命名规范），不得创建变体文件夹
7. **按章节导出**：批量导出时必须按章节分别导出，不得合并为单个文件
...
| `/novel-export [平台] ./正文/ --batch` | 批量转换目录 |
| `/novel-export [平台] ./正文/ --batch --concurrency 3` | 批量转换（自定义并发） |
| `/番茄格式 [文本]` | 快捷：番茄格式 |
| `/起点格式 [文本]` | 快捷：起点格式 |
